...
class Text {
    ...
    static protected function special_format_images($text) {
        $text = preg_replace_callback(
            "/\[img\:(\w[\w|\:|\.|\-%]+)\](\w[\w|\.|\-]+)/",
            function ($match) {
                return "<img src=".'"'."file.php?module=matrix&type=MatrixImage&file_id=".Text::getpic($match[2]).'"'." ".self::pic_parameters($match[1]).'>';
            },
            $text
        );
        $text = preg_replace_callback(
            "/\[img\](\w[\w|\.|\-%]+)/",
            function ($match) {
                return "<img src=".'"'."file.php?module=matrix&type=MatrixImage&file_id=".Text::getpic($match[1]).'"'.">";
            },
            $text
        );
        return $text;
    ...
    {
        $text = preg_replace('/\&sect;\&sect;(.*?)\&sect;\&sect;/', "", $text);
        $text = preg_replace_callback(
            "/\[img\:(\w[\w|\:|\.|\-%]+)\](\w[\w|\.|\-]+)/",
            function ($match) {
                return "<img src=" . '"' . "tmp/pdf_image_" . Text::getpic($match[2]) . '"' . " " . Text::pic_parameters($match[1]) . ">";
            },
            $text
        );
        $text = preg_replace("/\[img\](\w[\w|\.|\-%]+)/",
            function ($match) {
                return "<img src=" . '"' . "tmp/pdf_image_".Text::getpic($match[1]). '"' . ">";
            },
            $text
        );
